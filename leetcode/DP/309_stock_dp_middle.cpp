...
class Solution {
public:
    /*当price数组元素达到1000时，会超时，因为本算法复杂度为n^3*/
#ifdef MYSELF_TIMOUT    
            ...
            for (int j = i + 1; j < n; j++)
            {
                if (prices[j] < prices[i])
                    ...
                    for (int op = k - 1; op >= 0; --op)
                    {
                        ...
                        if(i>0)
                            dp[j + 1][op] = max(dp[j + 1][op], dp[i-1][op + 1] + prices[j] - prices[i]); //i->j天的盈利,前面应当有冷冻期
                        else{
                            dp[j + 1][op] = max(dp[j + 1][op], dp[i][op + 1] + prices[j] - prices[i]); //i->j天的盈利，一开始可以没有冷冻期
                        }
    ...
    int maxProfit(vector<int>& prices) {//NOTICE: 空间优化可到O(1)
        int n = prices.size();
